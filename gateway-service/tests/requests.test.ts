...
import fs from "fs";
import fetchNative, { RequestInit, Response } from "node-fetch";
...
// @ lambda cold start can take a while, causing occasional HTTP
// 502 failures. This function retries the request with exponential backoff.
async function fetch(url: URL | string, options?: RequestInit): Promise<Response> {
  const maxAttempts = 5;
  let result: Response;
...
describe("Test CORS requests", () => {
  ...
  test("CORS to /some/path (or any route) should succeed", async () => {
    const resp = await fetch(`${sam!.url.href}/some/path`, {
      method: "OPTIONS",
    });
    expect(resp.status).toEqual(200);
    expect(resp.headers.get("Access-Control-Allow-Origin")).toEqual("*");
  ...
  test(`Send request to http://example.localhost/echo and check the headers are passed through`, async () => {
    ...
    const resp = await fetch(requestUrl.href, {
      ...
      headers: [
        ...
        ["Custom-Header-Multi", "Custom-Header-Multi0"],
        ["Custom-Header-Multi", "Custom-Header-Multi1"],
      ],
      timeout: 10_000,
    });
...
async function startLocalSAM(): Promise<SAM> {
  ...
  try {
    ...
    try {
      sam.stdout.on("data", (data: string) => {
        console.log(`SAM: ${data}`);
      });

      sam.stderr.on("data", (data: string) => {
        console.log(`SAM: ${data}`);
      });
...
async function createUserAndSubscribeToExampleApp(
  ...
  const testUser = await createTestUser(context, { email: testUserEmail });
