...
import * as appFunctions from "@/src/functions/app";
...
describe("updateApp", () => {
  ...
  test("When updating an app, the search index is updated.", async () => {
    ...
    expect(updateAppSearchIndexSpy).toHaveBeenCalledWith(expect.anything(), [expect.objectContaining(updateVariables)]);
  });
