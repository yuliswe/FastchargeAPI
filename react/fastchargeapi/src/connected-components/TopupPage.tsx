...
class _TopUp extends React.Component<_Props, _State> {
    ...
    getSuccessUrl(): string {
        let url = new URLSearchParams(document.location.search).get("success_url");
    ...
    getCancelUrl(): string {
        let url = new URLSearchParams(document.location.search).get("cancel_url");
        if (url && !this.urlIsAllowed(url)) {
    ...
    isCanceled(): boolean {
        return new URLSearchParams(document.location.search).get("cancel") != null;
    }
    getAmountCents(): number {
        return parseInt(new URLSearchParams(document.location.search).get("amount_cents") || "0");
    }
    ...
    getBackendUrl(): string {
        let url = new URL("http://localhost:3000/checkout")
        return url.href;
    }
    ...
    async componentDidMount() {
        console.log(this.getSuccessUrl())
        console.log(this.getCancelUrl())
        ...
        } else { // Otherwise start the topuping process
    ...
    renderSuccessPage() {
        return <Typography>TopUping successful. You can close this page.</Typography>
    }

    renderCancelPage() {
        return <Typography>TopUping canceled. You can close this page.</Typography>
    }

    renderLoadingPage() {
        return <React.Fragment>
            <Typography>TopUping your account with Stripe. This could take up to a minute. Please wait.</Typography>
            <CircularProgress />
        </React.Fragment>
    }
    ...
    render() {
        return (
            <React.Fragment>
                ...
                {!this.isSuccess() && !this.isCanceled() && this.renderLoadingPage()}
            </React.Fragment>
        );
    }
}
