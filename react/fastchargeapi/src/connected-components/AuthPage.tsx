...
function postForm(path: string, params: any, method = 'post') {
...
class _AuthPage extends React.Component<_Props, _State> {
    ...
    getRedirectUrl(): string {
        let redirectUrl = new URLSearchParams(document.location.search).get("redirect") || ""
        if (!redirectUrl) {
        ...
        if (url.hostname !== window.location.hostname && url.hostname !== "localhost") {
            ...
            throw Error("Redirect URL must be the same host or localhost: " + redirectUrl)
        }
        return url.href
    }

    get behavior(): "redirect" | "redirect_and_close" {
        return (new URLSearchParams(document.location.search).get("behavior") || "redirect") as typeof this.behavior;
    };
    ...
    componentDidMount(): void {
        ...
        firebase.auth().onAuthStateChanged(async (user) => {
            if (user) { // user is signed in
                ...
                if (redirect) {
                    try {
                        await axios.post(redirect, { idToken, refreshToken: user.refreshToken })
                    } catch { } finally {
                        if (this.behavior === "redirect_and_close") {
                            window.close()
                        }
                    }
                }
